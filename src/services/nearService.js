...
async function getTransactionHistory(address) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('[getTransactionHistory] Response keys:', Object.keys(response.data || {}));
      console.log('[getTransactionHistory] Txns type:', typeof response.data?.txns);
      if (response.data?.txns) {
        console.log('[getTransactionHistory] Txns length:', response.data.txns.length);
        if (response.data.txns.length > 0) {
          console.log('[getTransactionHistory] First txn keys:', Object.keys(response.data.txns[0]));
        }
      }
    }
    ...
    // Если txns - объект (а не массив), преобразуем в массив
    if (txns && typeof txns === 'object' && !Array.isArray(txns)) {
    ...
    console.error('getTransactionHistory full error:', error);
